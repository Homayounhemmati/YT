...
SCENARIOS = {
    "year1": {"label": "Year 1 — young domain", "hard": 25, "medium": 18, "easy": 12},
    "year2": {"label": "Year 2 — early authority", "hard": 14, "medium": 10, "easy": 6},
    "year3": {"label": "Year 3 — established", "hard": 8, "medium": 6, "easy": 4},
}
...
def main():
    ...
    L = []
    w = L.append
    w("# Traffic and revenue model\n")
    w("> **Generated by `scripts/model_revenue.py` from `data/keywords.json`.** Do not edit by hand.\n")
    w(f"> Keywords measured: {data['measuredAt']} · market: {data['market']['location']}\n")
    ...
    w("\n## Assumptions\n")
    w("| Assumption | Value | Replaced by |")
    w("|---|---|---|")
    w(f"| AI Overview penalty | {opts['aiOverviewPenalty']:.0%} | a SERP check per keyword |")
    w(f"| Pages per session | {opts['sessionDepth']} | analytics, from month 2 |")
    w(f"| RPM multiplier from CPC | ×{opts['rpmMultiplier']} | the AdSense dashboard, from month 3 |")
    w("| Positions | the scenario table below | Search Console, from month 3 |")
    w("\n**The least certain number is the RPM multiplier.** The sensitivity analysis below exists for exactly that reason.\n")

    w("\n## The cluster\n")
    w("| Metric | Value |")
    w("|---|---|")
    w(f"| Monthly cluster volume | **{fmt(base['clusterVolume'])}** |")
    w(f"| Weighted average CPC | **${base['weightedCpc']:.2f}** |")
    w(f"| Implied cluster RPM | ${base['clusterRpm']:.2f} |")
    w(f"| Tools | {sum(1 for r in rows if r['kind']=='tool')} |")
    w(f"| Programmatic pages | {sum(r['pages'] for r in rows if r['kind']=='programmatic')} |")

    w("\n## Scenarios\n")
    w("| Scenario | Position (hard/medium/easy) | Entries/mo | Visits/mo | Share | Revenue/mo |")
    w("|---|---|---|---|---|---|")
    ...
    w(f"\n### When we reach ${target_low}-{target_high}\n")
    for name, sc in SCENARIOS.items():
        r = results[name]
        mark = "✅" if r["revenue"] >= target_low else "❌"
        w(f"- {mark} **{sc['label']}**: ${fmt(r['revenue'])}/month")

    w("\n## Sensitivity analysis — the RPM multiplier\n")
    w("| Multiplier | Year 1 | Year 2 | Year 3 |")
    w("|---|---|---|---|")
    ...
    w("The depth the funnel in section 2-2-2 creates is directly a revenue multiplier.\n")
    w("| Pages per session | Year 2 | Difference from 1.1 |")
    w("|---|---|---|")
    ...
    w("| Tool | Stage | Volume | CPC | Position | Entries/mo | Revenue/mo |")
    w("|---|---|---|---|---|---|---|")
    ...
    ex = data["excluded"]
    w(f"{len(ex)} keywords totalling **{fmt(sum(e['volume'] for e in ex))}** monthly searches:\n")
    w("| Keyword | Volume | CPC | Reason |")
    w("|---|---|---|---|")
